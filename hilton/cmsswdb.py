...
import configparser
import oracledb
#init thick client (requires Oracle instantclient libraries)
...
class CmsswDB(object):
    ...
    def __init__(self, database = None):
        ...
        try:
            ...
            print('%(command)s: error reading the configuration from %(config)s: %(error)s' % { 'command': self.command, 'config': self.config, 'error': str(sys.exc_info()) })
            sys.exit(1)
        try:
