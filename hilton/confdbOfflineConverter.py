...
class OfflineConverter:
    ...
    databases['daq'] = dict(databases['v3'])
...
def help():
    sys.stdout.write("""Usage: %s OPTIONS
        ...
        --v1|--v2|--v3|--v3-beta|--v3-test|--daq  (specify the ConfDB version [default: v3])
...
def main():
    ...
    _dbs['daq'] = [ '--%s' % _db for _db in OfflineConverter.databases['daq'] ] + [ '--runNumber' ]
    ...
    if _dbargs:
        ...
        if not db in OfflineConverter.databases[version]:
            ...
            sys.exit(1)
    #print(version, db, verbose, proxy, proxy_host,proxy_port)
